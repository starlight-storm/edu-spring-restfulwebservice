...
public class RestControllerExceptionHandler {
	...
	private static final String HTTP_LOCALHOST_8080_API_V1_EMPLOYEES = "http://localhost:8080/api/v1/employees";

	@org.springframework.web.bind.annotation.ExceptionHandler({ RuntimeException.class })
    @ResponseStatus(value = HttpStatus.INTERNAL_SERVER_ERROR)
    RestControllerError notFoundException(RuntimeException e) {
        return new RestControllerError(e.getMessage(), HTTP_LOCALHOST_8080_API_V1_EMPLOYEES);
    }
		...
		List<RestControllerError> errors = new ArrayList<RestControllerError>();
		  for(ObjectError err : e.getAllErrors()){
		      errors.add(new RestControllerError(err.getDefaultMessage(),
		    		  HTTP_LOCALHOST_8080_API_V1_EMPLOYEES));
		  }
        return errors;
    }
}